...
Public Class FRM_TKGA_FinalBilling
    ...
    Private Sub FRM_TKGA_FinalBilling_Load(sender As Object, e As EventArgs) Handles Me.Load
        ...
        sSGrid.Location = New Point(10, 250)
        sSGrid.Width = (Screen.PrimaryScreen.WorkingArea.Size.Width * 60) / 100
        Call Resize_Form()
        ...
        If gCenterlized = "N" Then
            StrSql = "SELECT ISNULL(KOTTYPE,'') AS KOTTYPE,ISNULL(PAYMENTMODE,'') AS PAYMENTMODE,ISNULL(FINALPREFIX,'') AS FINALPREFIX,ISNULL(TABLEREQUIRED,'') AS TABLEREQ,ISNULL(roundoffyesno,'') AS roundoffyesno,ISNULL(roundval,0) AS roundval FROM POSMASTER  WHERE POSCODE = '" & STRPOSCODE & "'"
        ...
        Try
            ...
            If gdataset.Tables("SM_CARDFILE_HDR").Rows.Count > 0 Then
                ...
                If gdataset.Tables("sm_bal").Rows.Count > 0 Then
                    ...
                    CARDOPBAL = Val(gdataset.Tables("sm_bal").Rows(0).Item("balance"))
                End If
    ...
    Private Sub Cmd_Add_Click(sender As Object, e As EventArgs) Handles Cmd_Add.Click
        ...
        Try
            ...
            'ssql = "select * from BILL_HDR  WHERE BillDate = '" & Format(CDate(Me.dtp_BillDate.Value), "dd-MMM-yyyy") & "'  AND ISNULL(accountflag,'') = 'Y'"
            'gconnection.getDataSet(ssql, "AccCheck")
            'If gdataset.Tables("AccCheck").Rows.Count > 0 Then
            '    MsgBox("Bill Closed, Can't Add or Update for this Date")
            '    Exit Sub
            'End If
            ...
            If Mid(CStr(Cmd_Add.Text), 1, 1) = "A" Then
                    ...
                    For Rau = 0 To gdataset.Tables("KTD").Rows.Count - 1
                        ...
                        If gdataset.Tables("ktd1").Rows.Count > 0 Then
                            ...
                            If Taxdr.Length > 0 Then
                                ...
                                Else
                                    sqlstring = "INSERT INTO Bill_Hdr(Billno,BillDetails,BillDate,BillTime,TaxAmount,BillAmount,Roundoff,PayMentmode,Paymentaccountcode,SubPaymentMode,Subpaymentaccountcode,Mcode,Mname,servicelocation,sname,CroStatus,Roomno,ChkId,Guest,AddUserId,AddDatetime,Upduserid,Upddatetime,Adjflag,Adjdate,Minimumusage,CardAmount,Remarks,CATEGORY,[16_digit_code],cardholdercode,cardholdername,PACKAMOUNT,TipsAmt,AdCgsAmt,PartyAmt,RoomAmt,sm_cardopbal) VALUES"
                                    If BILLROUNDYESNO = "YES" Then
                                        sqlstring = sqlstring & "('" & Trim(CStr(taxbillno)) & "','" & Trim(CStr(taxbilldetails)) & "','" & Format(CDate(Me.dtp_BillDate.Value), "dd-MMM-yyyy") & "','" & Format(Now, "T") & "'," & Val(TaxTotal) & "," & Math.Round(Val(TotalAmount)) & "," & Amt & ","
                                    Else
                                        sqlstring = sqlstring & "('" & Trim(CStr(taxbillno)) & "','" & Trim(CStr(taxbilldetails)) & "','" & Format(CDate(Me.dtp_BillDate.Value), "dd-MMM-yyyy") & "','" & Format(Now, "T") & "'," & Val(TaxTotal) & "," & Val(TotalAmount) & "," & Amt & ","
                                    End If
                                    sqlstring = sqlstring & "'" & Trim(Me.cbo_PaymentMode.Text) & "','" & Trim(paymentaccountcode) & " ','" & Trim(SubpaymentMode(0)) & "','" & Trim(Subpaymentaccountcode) & " ','" & Trim(Me.txt_MemberCode.Text) & "','" & Trim(Me.txt_MemberName.Text) & "','" & Trim(Me.txt_ServerCode.Text) & "',"
                                    sqlstring = sqlstring & "'','N',0,0,'','" & gUsername & "','" & Format(Now, "dd-MMM-yyyy hh:mm:ss") & "','','" & Format(Now, "dd-MMM-yyyy hh:mm:ss") & "','N','" & Format(Now, "dd-MMM-yyyy hh:mm:ss") & "',0,0,'" & Trim(CStr(Txt_Remarks.Text)) & "','BAR','" & txt_card_id.Text & "','" & txt_Holder_Code.Text & "','" & Txt_holder_name.Text & "'," & Val(TotPackAmt) & "," & Val(TotTipsAmt) & "," & Val(TotAdAmt) & "," & Val(TotPartyAmt) & "," & Val(TotRoomAmt) & "," & Val(CARDOPBAL) & ")"
                                End If
                                Update(j) = sqlstring
                                TaxApp = True
                ...
                If Mid(gCompName, 1, 3) = "KGA" Then
                    Dim SmsAmt As Double
                    If MessageBox.Show("Do You Want Send Sms of Bill ", MyCompanyName, MessageBoxButtons.OKCancel, MessageBoxIcon.Information, MessageBoxDefaultButton.Button1) = DialogResult.OK Then
                        sqlstring = "SELECT ISNULL(TotalAmount,0) AS TotalAmt FROM BILL_HDR WHERE BillDetails IN ( " & Trim(BillDetails) & ")"
                        gconnection.getDataSet(sqlstring, "BillAmt")
                        If gdataset.Tables("BillAmt").Rows.Count > 0 Then
                            SmsAmt = gdataset.Tables("BillAmt").Rows(0).Item(0)
                        End If
                        Call SendSMS(Trim(txt_MemberCode.Text), Math.Round(Val(SmsAmt)))
                    End If
                End If
                If CHK_PRINT.Checked = True Then
                    If UCase(Mid(gCompanyname, 1, 8)) = "KARNATAK" Then
    ...
    Private Sub CMB_BTYPE_SelectedIndexChanged(sender As Object, e As EventArgs) Handles CMB_BTYPE.SelectedIndexChanged
        ...
        End If
        If gCenterlized = "N" Then
            StrSql = "SELECT ISNULL(PACKINGPERCENT,0) AS PACKPER,ISNULL(TIPS,0) AS TIPS_SER,ISNULL(ADCHARGE,0) AS ADCHARGE,ISNULL(PRCHARGE,0) AS PRCHARGE,ISNULL(GRCHARGE,0) AS GRCHARGE,ISNULL(KOTTYPE,'') AS KOTTYPE,ISNULL(PAYMENTMODE,'') AS PAYMENTMODE,ISNULL(finalprefix,'') AS KOTPREFIX,ISNULL(TABLEREQUIRED,'') AS TABLEREQ,ISNULL(roundoffyesno,'') AS roundoffyesno,ISNULL(roundval,0) AS roundval FROM POSMASTER  WHERE POSCODE = '" & STRPOSCODE & "'"
            gconnection.getDataSet(StrSql, "PSETUP")
            If gdataset.Tables("PSETUP").Rows.Count > 0 Then
                ...
                DefaultPayment = gdataset.Tables("PSETUP").Rows(0).Item("PAYMENTMODE")
                RoundYN = gdataset.Tables("PSETUP").Rows(0).Item("roundoffyesno")
                BILLROUNDYESNO = Trim(gdataset.Tables("PSETUP").Rows(0).Item("roundoffyesno"))
                RndVal = gdataset.Tables("PSETUP").Rows(0).Item("roundval")
            End If
        End If
        Call fillPayment_Mode()
    ...
    Public Sub BillValidate(ByVal billdetails As String)
        ...
        If Trim(billdetails) <> "" Then
            ...
            sqlstring = sqlstring & " ISNULL(H.ROOMNO,0) AS ROOMNO,ISNULL(H.CHKID,0) AS CHKID,ISNULL(H.SUBPAYMENTMODE,'') AS SUBPAYMENTMODE, ISNULL(H.DELFLAG,'') AS DELFLAG,ISNULL(H.accountflag,'') AS accountflag, ISNULL(H.REMARKS,'') AS REMARKS,isnull(h.reason,'') as reason,ISNULL(SBILLFLAG,'') AS SBILLFLAG,ISNULL(INSTYPE,'')AS INSTYPE,ISNULL(INSNO,'')AS INSNO "
            ...
            If gdataset.Tables("BILL_HDR").Rows.Count > 0 Then
                ...
                End If
                ''If Trim(gdataset.Tables("BILL_HDR").Rows(0).Item("accountflag") & "") = "Y" Then
                ''    Cmd_Add.Enabled = False
                ''    Cmd_Delete.Enabled = False
                ''Else
                ''    Cmd_Add.Enabled = True
    ...
    Private Sub cmdOk_Click(sender As Object, e As EventArgs) Handles cmdOk.Click
        ...
        If gdataset.Tables("PASSWORD").Rows.Count > 0 Then
            ...
            End If
        Else
            MsgBox("check passsword...", MsgBoxStyle.Critical)
            txtPass.Text = ""
            grpPass.Visible = False
    ...
    Public Sub MCLUB_PRINT()
        ...
        If Cmd_Add.Text = "Add [F7]" Then
            ...
            End If
        Else
            sqlstring = "SELECT ISNULL(sm_cardopbal,0) BALANCE FROM bill_hdr WHERE Billdetails= '" & Trim(CStr(txt_BillNo.Text)) & "'"
            gconnection.getDataSet(sqlstring, "balanceAmount")
            If gdataset.Tables("balanceAmount").Rows.Count > 0 Then
                CardOpBalance = Val(gdataset.Tables("balanceAmount").Rows(0).Item(0))
            End If
                ...
                For V = 1 To 1
                    ...
                    If gdataset.Tables("ReportTable").Rows.Count > 0 Then
                            ...
                            If Vrowcount = 0 Then
                                If headercount = 1 Then
                                    ...
                                    If gCompanyname <> "" Then
                                        ...
                                        Rowprint = Space(5) & Chr(27) + "E" & gCompanyname & Chr(27) + "F"
                        ...
                        If BILLROUNDYESNO = "YES" Then
                            ...
                            End If
                        Else
                            Amt = (TaxAmount + TotalAmount + vPackAmt + vTipsAmt + vAdAmt + vPartyAmt + vRoomAmt) - (TaxAmount + TotalAmount + vPackAmt + vTipsAmt + vAdAmt + vPartyAmt + vRoomAmt)
                        End If
                        ...
                        Else
                            ...
                            Else
                                Rowprint = Rowprint & Format(((TaxAmount + TotalAmount + vPackAmt + vTipsAmt + vAdAmt + vPartyAmt + vRoomAmt)) + Val(Amt), "0.00")
                                toalamt = toalamt + Format(((TaxAmount + TotalAmount + vPackAmt + vTipsAmt + vAdAmt + vPartyAmt + vRoomAmt)) + Val(Amt), "0.00")
                            End If
                        End If
                        'If Amt < 0 Then
                        ...
                        For rowj = 0 To gdataset.Tables("ReportTable").Rows.Count - 1
                            ...
                            If Vrowcount = 0 Then
                                If headercount = 1 Then
                                    ...
                                    'Filewrite.WriteLine(Rowprint)
                                    'Rowprint = Space(3) & "BILL DATE :"
                                    Rowprint = Rowprint & Space(3) & "BILL DATE :"
                                    Rowprint = Rowprint & Format(gdataset.Tables("ReportTable").Rows(rowj).Item("BillDate"), "dd/MM/yyyy")
                                    Rowprint = Rowprint & " "
                                    Rowprint = Rowprint & Format(gdataset.Tables("ReportTable").Rows(rowj).Item("BillTime"), "T")
                                    Filewrite.WriteLine(Rowprint)
                                    ...
                                    If MCODE = "" Then
                                        ...
                                        Rowprint = Rowprint & Space(1) & Chr(15) & Trim(ROOMNO)
                                        'Filewrite.WriteLine(Rowprint & Chr(15))
                                        Rowprint = Rowprint & Space(3) & "GUEST NAME:" & Space(1) & Chr(15) & Trim(GUEST)
                                        Filewrite.WriteLine(Rowprint & Chr(17))
                                    Else
                                        Rowprint = Space(3) & "MEM CODE  :"
                                        Rowprint = Rowprint & Space(1) & Chr(15) & Trim(MCODE)
                                        'Filewrite.WriteLine(Rowprint & Chr(15))
                                        Rowprint = Rowprint & Space(3) & "MEM NAME  :" & Space(1) & Chr(15) & Trim(MNAME)
                                        Filewrite.WriteLine(Rowprint & Chr(17))
                                    End If
                                    If Trim(Table) <> "" Then
                                        Rowprint = Space(3) & "TABLE     :"
    ...
    Public Sub MCLUB_PRINTMCEME()
        ...
        If Cmd_Add.Text = "Add [F7]" Then
            CardOpBalance = CardOpBalance + trnBalance '+ Math.Round(OtrnBalance)
        Else
            CardOpBalance = CardOpBalance
        End If
        ...
        If gdataset.Tables("billing").Rows.Count > 0 Then
            For jo = 0 To gdataset.Tables("billing").Rows.Count - 1
                ...
                For V = 1 To 1
                    If V = 1 Then
                        If Trim(txt_ServerCode.Text) <> "" Then
                            sql = "SELECT BILL_HDR.BILLDETAILS AS HBILLDETAILS,BILL_HDR.PAYMENTMODE,BILL_HDR.MCODE,BILL_HDR.MNAME, BILL_HDR.CARDHOLDERCODE ,BILL_HDR.CARDHOLDERNAME ,    D.SCODE,E.SERVERNAME AS SNAME,"
                            'sql = sql & "D.BILLDETAILS,ITEMCODE,ITEMDESC,D.POSCODE,D.UOM,SUM(D.QTY) AS QTY,ISNULL(D.TABLENO,'') AS TABLENO,"
                            sql = sql & "D.BILLDETAILS,D.KOTNO,D.KOTDETAILS,ITEMCODE,ITEMDESC,D.POSCODE,D.UOM,SUM(D.QTY) AS QTY,ISNULL(D.TABLENO,'') AS TABLENO,"
                            sql = sql & "ISNULL(D.RATE,0) AS RATE,SUM(ISNULL(D.AMOUNT,0)) AS AMOUNT,TAXTYPE,TAXPERC,ISNULL(TAXCODE,'') as TAXCODE,SUM(ISNULL(D.TAXAMOUNT,0)) as TAXAMOUNT,SUM(ISNULL(D.PACKAMOUNT,0)) as PACKAMOUNT,SUM(ISNULL(D.TipsAmt,0)) as TipsAmt,SUM(ISNULL(D.AdCgsAmt,0)) as AdCgsAmt,SUM(ISNULL(D.PartyAmt,0)) as PartyAmt,SUM(ISNULL(D.RoomAmt,0)) as RoomAmt,"
                            ...
                            sql = sql & "  INNER JOIN SERVERMASTER E  ON E.SERVERCODE=D.SCODE  "
                            sql = sql & " WHERE D.Billdetails IN (" & Trim(billdetails) & ") AND ISNULL(KotStatus,'N')='N' "
                            sql = sql & " group by D.POSCODE,BILL_HDR.PAYMENTMODE,BILL_HDR.MCODE,BILL_HDR.MNAME, BILL_HDR.CARDHOLDERCODE ,BILL_HDR.CARDHOLDERNAME ,D.SCODE,E.SERVERNAME,BILL_HDR.DELFLAG,"
                            sql = sql & " D.BILLDETAILS,ITEMCODE,ITEMDESC,D.POSCODE,D.UOM,D.RATE,D.TABLENO,BILL_HDR.PRINTFLAG,"
                            ...
                            sql = sql & " --ORDER BY D.KOTDETAILS --D.BillDetails "
                        Else
                            sql = "SELECT BILL_HDR.BILLDETAILS AS HBILLDETAILS,BILL_HDR.PAYMENTMODE,BILL_HDR.MCODE,BILL_HDR.MNAME, BILL_HDR.CARDHOLDERCODE ,BILL_HDR.CARDHOLDERNAME ,D.SCODE,'' AS SNAME,"
                            'sql = sql & "D.BILLDETAILS,ITEMCODE,ITEMDESC,D.POSCODE,D.UOM,SUM(D.QTY) AS QTY,ISNULL(D.TABLENO,'') AS TABLENO,"
                            sql = sql & "D.BILLDETAILS,D.KOTNO,D.KOTDETAILS,ITEMCODE,ITEMDESC,D.POSCODE,D.UOM,SUM(D.QTY) AS QTY,ISNULL(D.TABLENO,'') AS TABLENO,"
                            sql = sql & "ISNULL(D.RATE,0) AS RATE,SUM(ISNULL(D.AMOUNT,0)) AS AMOUNT,TAXTYPE,TAXPERC,ISNULL(TAXCODE,'') as TAXCODE,SUM(ISNULL(D.TAXAMOUNT,0)) as TAXAMOUNT,SUM(ISNULL(D.PACKAMOUNT,0)) as PACKAMOUNT,SUM(ISNULL(D.TipsAmt,0)) as TipsAmt,SUM(ISNULL(D.AdCgsAmt,0)) as AdCgsAmt,SUM(ISNULL(D.PartyAmt,0)) as PartyAmt,SUM(ISNULL(D.RoomAmt,0)) as RoomAmt,"
                            'sql = sql & "ISNULL(D.ActualRate,0) AS RATE,SUM(ISNULL(D.ActualAmount,0)) AS AMOUNT,TAXTYPE,TAXPERC,ISNULL(TAXCODE,'') as TAXCODE,SUM(ISNULL(D.TAXAMOUNT,0)) as TAXAMOUNT,SUM(ISNULL(D.PACKAMOUNT,0)) as PACKAMOUNT,SUM(ISNULL(D.TipsAmt,0)) as TipsAmt,SUM(ISNULL(D.AdCgsAmt,0)) as AdCgsAmt,SUM(ISNULL(D.PartyAmt,0)) as PartyAmt,SUM(ISNULL(D.RoomAmt,0)) as RoomAmt,SUM(ISNULL(D.DAmt,0)) as DAmt,"
                            ...
                            'sql = sql & "  INNER JOIN SERVERMASTER E  ON E.SERVERCODE=D.SCODE  "
                            sql = sql & " WHERE D.Billdetails IN (" & Trim(billdetails) & ") AND ISNULL(KotStatus,'N')='N' "
                            sql = sql & " group by D.POSCODE,BILL_HDR.PAYMENTMODE,BILL_HDR.MCODE,BILL_HDR.MNAME,  BILL_HDR.CARDHOLDERCODE ,BILL_HDR.CARDHOLDERNAME ,  D.SCODE,BILL_HDR.DELFLAG,"
                            sql = sql & " D.BILLDETAILS,ITEMCODE,ITEMDESC,D.POSCODE,D.UOM,D.RATE,D.TABLENO,BILL_HDR.PRINTFLAG,"
                            'sql = sql & " D.BILLDETAILS,ITEMCODE,ITEMDESC,D.POSCODE,D.UOM,D.ActualRate,D.TABLENO,BILL_HDR.PRINTFLAG,"
                            sql = sql & " TAXTYPE,TAXPERC,TAXCODE,ITEMTYPE,BILLDATE,BILLTIME,BILL_HDR.adduserid,BILL_HDR.servicelocation,D.KOTNO,D.KOTDETAILS,"
                            sql = sql & "GUEST,ISNULL(CAST(ROOMNO AS VARCHAR),''),ISNULL(CAST(BILL_HDR.CHKID AS VARCHAR),''),BILL_HDR.BILLDETAILS"
                    ...
                    If gdataset.Tables("ReportTable").Rows.Count > 0 Then
                        ...
                        Dim SETMCODE, SQL2 As String
                        Dim SETAMOUNT As Double
                        Dim vpagenumber, Vrowcount, innercount, taxcount As Long
                        Dim headercount As Double
                        Dim ROOMNO As Integer
                        Dim SSQLROOM, poscode, PAYMENTMODE, MCODE, MNAME, GUEST, COURSE, CCODE, CNAME As String
                        COURSE = ""
                        innercount = 0
                        Vrowcount = 0
                        vpagenumber = 1
                        ...
                        MCODE = Trim(gdataset.Tables("ReportTable").Rows(0).Item("MCODE"))
                        MNAME = Trim(gdataset.Tables("ReportTable").Rows(0).Item("MNAME"))
                        CCODE = Trim(gdataset.Tables("ReportTable").Rows(0).Item("CARDHOLDERCODE"))
                        CNAME = Trim(gdataset.Tables("ReportTable").Rows(0).Item("CARDHOLDERNAME"))
                        GUEST = Trim(gdataset.Tables("ReportTable").Rows(0).Item("GUEST"))
                        ROOMNO = Trim(gdataset.Tables("ReportTable").Rows(0).Item("ROOMNO"))
                        HBILLDETAILS = Trim(gdataset.Tables("ReportTable").Rows(0).Item("HBILLDETAILS"))
                        BILLPLACE = Trim(gdataset.Tables("ReportTable").Rows(0).Item("HBILLDETAILS"))
                        ...
                        SQL2 = "SELECT MEMBERTYPE,CASE WHEN COURSE='' THEN MEMBERTYPE ELSE ISNULL(COURSE,MEMBERTYPE) END AS COURSE FROM MEMBERMASTER WHERE MCODE='" & MCODE & "'"
                        gconnection.getDataSet(SQL2, "MEMBERMASTER")
                        If gdataset.Tables("ReportTable").Rows.Count > 0 Then
                            COURSE = Trim(gdataset.Tables("MEMBERMASTER").Rows(0).Item("COURSE"))
                        End If
                            ...
                            If Vrowcount = 0 Then
                                If headercount = 1 Then
                                    ...
                                    If gCompanyname <> "" Then
                                        ...
                                        'Vrowcount = Vrowcount + 1
                                        Rowprint = Space(5) & Chr(27) & Chr(27) + "" & gCompanyname & Chr(27) + "F"
                                    ...
                                    If MCODE = "" Then
                                        Rowprint = Space(3) & "ROOM NO   :"
                                        'Rowprint = Rowprint & Space(1) & Chr(15) & Trim(ROOMNO)
                                        Rowprint = Rowprint & Space(1) & Trim(ROOMNO)
                                        'Filewrite.WriteLine(Rowprint & Chr(15))
                                        Filewrite.WriteLine(Rowprint)
                                        'Rowprint = Space(3) & "GUEST NAME:" & Space(1) & Chr(15) & Trim(GUEST)
                                    ...
                                    Else
                                        Rowprint = Space(3) & "MEM CODE  :"
                                        'Rowprint = Rowprint & Space(1) & Chr(15) & Trim(MCODE)
                                        Rowprint = Rowprint & Space(1) & Trim(MCODE)
                                        Rowprint = Rowprint & "," & Space(1) & "TYPE: " & COURSE
                                        Filewrite.WriteLine(Rowprint)
                                        If Trim(CCODE) = "" Then
                                            Rowprint = Space(3) & "MEM NAME  :" & Space(1) & Trim(MNAME)
                                            Filewrite.WriteLine(Rowprint & Chr(17))
                                        Else
                                            Rowprint = Space(3) & "CARD-CODE :" & Space(1) & Trim(CCODE)
                                            Filewrite.WriteLine(Rowprint & Chr(17))
                                            Rowprint = Space(3) & "CARD-NAME :" & Space(1) & Trim(CNAME)
                                            Filewrite.WriteLine(Rowprint & Chr(17))
                                        End If
                                    End If
                                    If Trim(Table) <> "" Then
                                    ...
                                    End If
                                    If gCompanyname <> "" Then
                                        Filewrite.WriteLine(Space(3) & StrDup(40, "-"))
                                        ''Filewrite.WriteLine()
                                        Vrowcount = Vrowcount + 1
                        ...
                        Else
                            ...
                            Rowprint = Rowprint & Format(((TaxAmount + TotalAmount + vPackAmt + vTipsAmt + vAdAmt + vPartyAmt + vRoomAmt - VDisAmt)) + Val(Amt), "0.00")
                            toalamt = toalamt + Format(((TaxAmount + TotalAmount + vPackAmt + vTipsAmt + vAdAmt + vPartyAmt + vRoomAmt - VDisAmt)) + Val(Amt), "0.00")
                        End If
                        'If Amt < 0 Then
                        '    Rowprint = Rowprint & Format(Val((TaxAmount + TotalAmount + vPackAmt + vTipsAmt + vAdAmt + vPartyAmt + vRoomAmt)) - Val(Amt), "0.00")
                        '    toalamt = toalamt + Format(Val((TaxAmount + TotalAmount + vPackAmt + vTipsAmt + vAdAmt + vPartyAmt + vRoomAmt)) - Val(Amt), "0.00")
                        ...
                        If Vrowcount = 35 Or Vrowcount = 38 Or Vrowcount = 63 Then
                            ...
                            Next
                        End If
                        Filewrite.WriteLine(Space(3) & Chr(15) & Chr(27) & StrDup(40, "-"))
                        Vrowcount = Vrowcount + 1
                        '11
                        taxcount = taxcount + 1
                        If Vrowcount = 35 Or Vrowcount = 38 Or Vrowcount = 63 Then
                        ...
                        If Len(Kot) < 31 Then
                            ...
                            '13
                        Else
                            Rowprint = Space(3) & "ORDER's :" & Mid(Kot, 1, 28) & ","
                            Filewrite.WriteLine(Rowprint)
                            Rowprint = "             " & Mid(Kot, 29, 30) & "."
                            Filewrite.WriteLine(Rowprint)
                            Vrowcount = Vrowcount + 2
            ...
            For jo = 0 To gdataset.Tables("billing").Rows.Count - 1
                ...
                For V = 1 To 1
                    ...
                    If gdataset.Tables("ReportTable").Rows.Count > 0 Then
                        ...
                        For rowj = 0 To gdataset.Tables("ReportTable").Rows.Count - 1
                            ...
                            If Vrowcount = 0 Then
                                If headercount = 1 Then
                                    ...
                                    'Vrowcount = Vrowcount + 1
                                    Dim Bno As String()
                                    Bno = Split(HBILLDETAILS, "/")
                                    Rowprint = Chr(18) & Chr(27) & Chr(106) & Chr(200) & Chr(27) & Chr(14) & Chr(27) & Chr(71) & "" & Chr(27)
                                    Filewrite.Write(Chr(14) & Chr(15) & Rowprint & Chr(18))
                                    Filewrite.Write("" & Chr(15))
                                    'Filewrite.WriteLine("{0,-30}{1,-16}{2,-17}{3,-15}", gCompanyname, "Bill No:" & Bno(1), "Date:" & Format(gdataset.Tables("ReportTable").Rows(rowj).Item("BillDate"), "dd/MM/yyyy"), MCODE)
                                    'Filewrite.WriteLine("{0,-30}{1,-16}{2,-17}{3,-10}{4,5}", gCompanyname, "Bill No:" & Bno(1), "Date:" & Format(gdataset.Tables("ReportTable").Rows(rowj).Item("BillDate"), "dd/MM/yyyy"), MCODE, Bno(0))
                                    Filewrite.Write("{0,-30}", gCompanyname)
                                    Filewrite.Write("" & Chr(18) & Chr(15))
                                    Filewrite.WriteLine("{0,-16}{1,-17}{2,-10}{3,5}", "Bill No:" & Bno(1), "Date:" & Format(gdataset.Tables("ReportTable").Rows(rowj).Item("BillDate"), "dd/MM/yyyy"), MCODE, Bno(0))
                                    'Filewrite.WriteLine("{0,-30}{1,-16}{2,-17}{3,-15}", Address1 & "," & Address2 & "," & gCity & "-" & gPincode, "----------------", "-----------------", "---------------")
                                    Filewrite.WriteLine("{0,-30}{1,-16}{2,-17}{3,-15}", "1,Golf Avenue, Bangalore-8", "----------------", "-----------------", "---------------")
                                    Filewrite.WriteLine("{0,-30}{1,-7}{2,-17}{3,6}{4,7}{5,11}", "", "Req.No", "Discription", "Qty", "Rate", "Amount")
                                    Filewrite.WriteLine("{0,-30}{1,-16}{2,-17}{3,-15}", Space(5) & gconnection.getvalue("SELECT ISNULL(POSDESC,'') AS POSDESC FROM POSMASTER WHERE POSCODE='" & poscode & "'"), "----------------", "-----------------", "---------------")
                            ...
                            With gdataset.Tables("ReportTable").Rows(rowj)
                                If gCompanyname <> "" Then
                                    ...
                                    ''    Next
                                    ''End If
                                    'Filewrite.WriteLine("{0,-30}{1,-7}{2,-17}{3,6}{4,7}{5,11}", "", Mid(Trim(.Item("ITEMCODE")), 1, 6), Mid(Trim(.Item("ITEMDESC")), 1, 15), Mid(Val(.Item("QTY")), 1, 5), Mid(Format(Val(.Item("RATE")), "0.00"), 1, 7), Mid(Format(Val(.Item("AMOUNT")), "0.00"), 1, 10))

                                    If gdataset.Tables("ReportTable").Rows.Count >= 5 Then
                                        If rowj = 2 Then
                                            Filewrite.WriteLine("{0,-30}{1,-7}{2,-17}{3,6}{4,7}{5,11}", "TIN.29510048086", Mid(Trim(.Item("ITEMCODE")), 1, 6), Mid(Trim(.Item("ITEMDESC")), 1, 15), Mid(Val(.Item("QTY")), 1, 5), Mid(Format(Val(.Item("RATE")), "0.00"), 1, 7), Mid(Format(Val(.Item("AMOUNT")), "0.00"), 1, 10))
                                        ElseIf rowj = 4 Then
                                            Filewrite.WriteLine("{0,-30}{1,-7}{2,-17}{3,6}{4,7}{5,11}", "Time:" & Format(gdataset.Tables("ReportTable").Rows(rowj).Item("BillTime"), "T"), Mid(Trim(.Item("ITEMCODE")), 1, 6), Mid(Trim(.Item("ITEMDESC")), 1, 15), Mid(Val(.Item("QTY")), 1, 5), Mid(Format(Val(.Item("RATE")), "0.00"), 1, 7), Mid(Format(Val(.Item("AMOUNT")), "0.00"), 1, 10))
                                        Else
                                            Filewrite.WriteLine("{0,-30}{1,-7}{2,-17}{3,6}{4,7}{5,11}", "", Mid(Trim(.Item("ITEMCODE")), 1, 6), Mid(Trim(.Item("ITEMDESC")), 1, 15), Mid(Val(.Item("QTY")), 1, 5), Mid(Format(Val(.Item("RATE")), "0.00"), 1, 7), Mid(Format(Val(.Item("AMOUNT")), "0.00"), 1, 10))
                                        End If
                                    ElseIf gdataset.Tables("ReportTable").Rows.Count >= 3 And gdataset.Tables("ReportTable").Rows.Count < 5 Then
                                        If rowj = 2 Then
                                            Filewrite.WriteLine("{0,-30}{1,-7}{2,-17}{3,6}{4,7}{5,11}", "TIN.29510048086", Mid(Trim(.Item("ITEMCODE")), 1, 6), Mid(Trim(.Item("ITEMDESC")), 1, 15), Mid(Val(.Item("QTY")), 1, 5), Mid(Format(Val(.Item("RATE")), "0.00"), 1, 7), Mid(Format(Val(.Item("AMOUNT")), "0.00"), 1, 10))
                                        Else
                                            Filewrite.WriteLine("{0,-30}{1,-7}{2,-17}{3,6}{4,7}{5,11}", "", Mid(Trim(.Item("ITEMCODE")), 1, 6), Mid(Trim(.Item("ITEMDESC")), 1, 15), Mid(Val(.Item("QTY")), 1, 5), Mid(Format(Val(.Item("RATE")), "0.00"), 1, 7), Mid(Format(Val(.Item("AMOUNT")), "0.00"), 1, 10))
                        ...
                        Next rowj
                        taxcount = 0
                        If gdataset.Tables("ReportTable").Rows.Count = 1 Then
                            Filewrite.WriteLine()
                            Filewrite.WriteLine("{0,-30}{1,-7}{2,-17}{3,6}{4,7}{5,11}", "TIN.29510048086", "", "", "", "", "")
                            Filewrite.WriteLine()
                            Filewrite.WriteLine("{0,-30}{1,-7}{2,-17}{3,6}{4,7}{5,11}", "Time:" & Format(gdataset.Tables("ReportTable").Rows(0).Item("BillTime"), "T"), "", "", "", "", "")
                        End If
                        If gdataset.Tables("ReportTable").Rows.Count = 2 Then
                            Filewrite.WriteLine("{0,-30}{1,-7}{2,-17}{3,6}{4,7}{5,11}", "TIN.29510048086", "", "", "", "", "")
                            Filewrite.WriteLine()
                            Filewrite.WriteLine("{0,-30}{1,-7}{2,-17}{3,6}{4,7}{5,11}", "Time:" & Format(gdataset.Tables("ReportTable").Rows(0).Item("BillTime"), "T"), "", "", "", "", "")
                        End If
                        If gdataset.Tables("ReportTable").Rows.Count = 3 Then
                            Filewrite.WriteLine()
                            Filewrite.WriteLine("{0,-30}{1,-7}{2,-17}{3,6}{4,7}{5,11}", "Time:" & Format(gdataset.Tables("ReportTable").Rows(0).Item("BillTime"), "T"), "", "", "", "", "")
                        End If
                        If gdataset.Tables("ReportTable").Rows.Count = 4 Then
                            Filewrite.WriteLine("{0,-30}{1,-7}{2,-17}{3,6}{4,7}{5,11}", "Time:" & Format(gdataset.Tables("ReportTable").Rows(0).Item("BillTime"), "T"), "", "", "", "", "")
                        End If

                        If gCompanyname <> "" Then
                            'Filewrite.WriteLine(Space(3) & StrDup(45, "-"))
                            Filewrite.WriteLine("{0,-30}{1,-16}{2,-17}{3,-15}", "", "----------------", "-----------------", "---------------")
                        End If
                        'If Vrowcount = 21 Or Vrowcount = 51 Then
                        '    For i = 0 To 10
                        '        Filewrite.WriteLine()
                        ...
                        'Rowprint = Space(3) & "Total:" & Space(23) & Space(8 - Len(Mid(Trim(Format(TotalAmount, "0.00")), 1, 8))) & Mid(Trim(Format(TotalAmount, "0.00")), 1, 8)
                        'Filewrite.WriteLine(Rowprint)
                        Filewrite.WriteLine("{0,-30}{1,-33}{2,15}", MNAME, "TOTAL AMOUNT", Mid(Trim(Format(TotalAmount, "0.00")), 1, 10))
                        Vrowcount = Vrowcount + 1
                        'If Vrowcount = 35 Or Vrowcount = 38 Or Vrowcount = 63 Then
                        '    For i = 1 To 6
                        '        Filewrite.WriteLine()
                        ...
                        If gCompanyname <> "" Then
                            ...
                            For i = 0 To gdataset.Tables("TaxDet").Rows.Count - 1
                                ...
                                'Filewrite.WriteLine(Rowprint & Chr(17))
                                If i = 0 Then
                                    Filewrite.WriteLine("{0,-30}{1,-33}{2,15}", "A/C No." & MCODE, Mid(gdataset.Tables("TaxDet").Rows(i).Item("TAXDESC"), 1, 25), Mid(Format(Val(gdataset.Tables("TaxDet").Rows(i).Item("taxamount") & ""), "0.00"), 1, 10))
                                Else
                                    Filewrite.WriteLine("{0,-30}{1,-33}{2,15}", "", Mid(gdataset.Tables("TaxDet").Rows(i).Item("TAXDESC"), 1, 25), Mid(Format(Val(gdataset.Tables("TaxDet").Rows(i).Item("taxamount") & ""), "0.00"), 1, 10))
                                End If
                                taxcount = taxcount + 1
                                Vrowcount = Vrowcount + 1
                                'If Vrowcount = 35 Or Vrowcount = 38 Or Vrowcount = 63 Then
                        ...
                        If vPackAmt > 0 Then
                            ...
                            'Rowprint = Rowprint & Space(8 - Len(Mid(Format(Val(vPackAmt & ""), "0.00"), 1, 8))) & Mid(Format(Val(vPackAmt & ""), "0.00"), 1, 8)
                            'Filewrite.WriteLine(Rowprint & Chr(17))
                            Filewrite.WriteLine("{0,-30}{1,-33}{2,15}", "", Mid(PackDesc, 1, 25), Mid(Format(Val(vPackAmt & ""), "0.00"), 1, 10))
                            taxcount = taxcount + 1
                            Vrowcount = Vrowcount + 1
                        End If
                        If vTipsAmt > 0 Then
                            ...
                            'Rowprint = Rowprint & Space(8 - Len(Mid(Format(Val(vTipsAmt & ""), "0.00"), 1, 8))) & Mid(Format(Val(vTipsAmt & ""), "0.00"), 1, 8)
                            'Filewrite.WriteLine(Rowprint & Chr(17))
                            Filewrite.WriteLine("{0,-30}{1,-33}{2,15}", "", Mid(TipsDesc, 1, 25), Mid(Format(Val(vTipsAmt & ""), "0.00"), 1, 10))
                            taxcount = taxcount + 1
                            Vrowcount = Vrowcount + 1
                        End If
                        If vAdAmt > 0 Then
                            ...
                            'Rowprint = Rowprint & Space(8 - Len(Mid(Format(Val(vAdAmt & ""), "0.00"), 1, 8))) & Mid(Format(Val(vAdAmt & ""), "0.00"), 1, 8)
                            'Filewrite.WriteLine(Rowprint & Chr(17))
                            Filewrite.WriteLine("{0,-30}{1,-33}{2,15}", "", Mid(AddChgDesc, 1, 25), Mid(Format(Val(vAdAmt & ""), "0.00"), 1, 10))
                            taxcount = taxcount + 1
                            Vrowcount = Vrowcount + 1
                        End If
                        If vPartyAmt > 0 Then
                            ...
                            'Rowprint = Rowprint & Space(8 - Len(Mid(Format(Val(vPartyAmt & ""), "0.00"), 1, 8))) & Mid(Format(Val(vPartyAmt & ""), "0.00"), 1, 8)
                            'Filewrite.WriteLine(Rowprint & Chr(17))
                            Filewrite.WriteLine("{0,-30}{1,-33}{2,15}", "", "Add Party Charges", Mid(Format(Val(vPartyAmt & ""), "0.00"), 1, 10))
                            taxcount = taxcount + 1
                            Vrowcount = Vrowcount + 1
                        End If
                        If vRoomAmt > 0 Then
                            ...
                            'Rowprint = Rowprint & Space(8 - Len(Mid(Format(Val(vRoomAmt & ""), "0.00"), 1, 8))) & Mid(Format(Val(vRoomAmt & ""), "0.00"), 1, 8)
                            'Filewrite.WriteLine(Rowprint & Chr(17))
                            Filewrite.WriteLine("{0,-30}{1,-33}{2,15}", "", "Add Room Charges", Mid(Format(Val(vRoomAmt & ""), "0.00"), 1, 10))
                            taxcount = taxcount + 1
                            Vrowcount = Vrowcount + 1
                        End If
                        ...
                        If Amt < 0 Then
                            'Rowprint = Rowprint & Format(Val((TaxAmount + TotalAmount + vPackAmt + vTipsAmt + vAdAmt + vPartyAmt + vRoomAmt)) - Val(Amt * -1), "0.00")
                            toalamt = toalamt + Format(Val((TaxAmount + TotalAmount + vPackAmt + vTipsAmt + vAdAmt + vPartyAmt + vRoomAmt)) - Val(Amt * -1), "0.00")
                            Filewrite.WriteLine("{0,-30}{1,-33}{2,15}", "Cashier: " & addUser, "NET AMOUNT (ROUND)", Format(Val((TaxAmount + TotalAmount + vPackAmt + vTipsAmt + vAdAmt + vPartyAmt + vRoomAmt)) - Val(Amt * -1), "0.00"))
                        Else
                            If BILLROUNDYESNO = "YES" Then
                                'Rowprint = Rowprint & Format(Math.Round((TaxAmount + TotalAmount + vPackAmt + vTipsAmt + vAdAmt + vPartyAmt + vRoomAmt)) + Val(Amt), "0.00")
                                toalamt = toalamt + Format(Math.Round((TaxAmount + TotalAmount + vPackAmt + vTipsAmt + vAdAmt + vPartyAmt + vRoomAmt)) + Val(Amt), "0.00")
                                Filewrite.WriteLine("{0,-30}{1,-33}{2,15}", "Cashier: " & addUser, "NET AMOUNT (ROUND)", Format(Math.Round((TaxAmount + TotalAmount + vPackAmt + vTipsAmt + vAdAmt + vPartyAmt + vRoomAmt)) + Val(Amt), "0.00"))
                            Else
                                'Rowprint = Rowprint & Format(((TaxAmount + TotalAmount + vPackAmt + vTipsAmt + vAdAmt + vPartyAmt + vRoomAmt)) + Val(Amt), "0.00")
                                toalamt = toalamt + Format(((TaxAmount + TotalAmount + vPackAmt + vTipsAmt + vAdAmt + vPartyAmt + vRoomAmt)) + Val(Amt), "0.00")
                                Filewrite.WriteLine("{0,-30}{1,-33}{2,15}", "Cashier: " & addUser, "NET AMOUNT (ROUND)", Format(((TaxAmount + TotalAmount + vPackAmt + vTipsAmt + vAdAmt + vPartyAmt + vRoomAmt)) + Val(Amt), "0.00"))
                            End If
                        End If
                        'If Amt < 0 Then
                        '    Rowprint = Rowprint & Format(Val((TaxAmount + TotalAmount + vPackAmt + vTipsAmt + vAdAmt + vPartyAmt + vRoomAmt)) - Val(Amt), "0.00")
                        ...
                        'End If
                        'Filewrite.WriteLine(Rowprint & Chr(15))
                        'Filewrite.WriteLine("{0,-30}{1,-33}{2,-15}", "", "NET AMOUNT (ROUND)", Mid(Format(Val(vRoomAmt & ""), "0.00"), 1, 10))
                        '10
                        taxcount = taxcount + 1
                        Vrowcount = Vrowcount + 1
                        'If Vrowcount = 35 Or Vrowcount = 38 Or Vrowcount = 63 Then
                        ...
                        'End If
                        ''Filewrite.WriteLine(Space(3) & Chr(15) & Chr(27) & StrDup(45, "-"))
                        Filewrite.WriteLine("{0,-30}{1,-16}{2,-17}{3,-15}", "Payment Type:" & Trim(cbo_PaymentMode.Text), "----------------", "-----------------", "---------------")
                        Vrowcount = Vrowcount + 1
                        '11
                        taxcount = taxcount + 1
                        'If Vrowcount = 35 Or Vrowcount = 38 Or Vrowcount = 63 Then
    ...
    Private Sub Cmd_View_Click(sender As Object, e As EventArgs) Handles Cmd_View.Click
        If Mid(Trim(UCase(gCompanyname)), 1, 10) = "THE BENGAL" Then
            Call Bengal_Print()
        ElseIf Mid(gCompName, 1, 5) = "MCEME"or Mid(gCompName, 1, 4) = "ASCC" Then
            gPrint = False
            Call MCLUB_PRINTMCEME()
        ElseIf Mid(gCompName, 1, 3) = "KGA" Then
            gPrint = False
            Call MCLUB_PRINT_KGA()
        Else
            gPrint = False
            Call MCLUB_PRINT()
        End If
    ...
    Private Sub Bengal_Print()
        Dim sqlstring, SSQL, LocName As String
        ...
        sqlstring = "SELECT locationName FROM locationMaster WHERE locationCode IN (SELECT servicelocation FROM KOT_HDR WHERE Kotdetails IN (SELECT Kotdetails FROM KOT_det WHERE BILLDETAILS = '" & Trim(txt_BillNo.Text) & "'))"
        gconnection.getDataSet(sqlstring, "loc")
        If gdataset.Tables("loc").Rows.Count > 0 Then
            LocName = gdataset.Tables("loc").Rows(0).Item(0)
        Else
            LocName = ""
        End If
        sqlstring = "select K.ITEMCODE,K.ITEMDESC,K.RATE,K.QTY,AMOUNT,VAT AS TAXAMOUNT,SER AS TAXAMT,ISNULL(PACKAMOUNT,0)+isnull(TipsAmt,0)+isnull(AdCgsAmt,0)+isnull(PartyAmt,0)+isnull(RoomAmt,0) AS Packamount,'" & Trim(LocName) & "' as Taxtype "
    ...
    Private Sub Cmd_PendingBill_Click(sender As System.Object, e As System.EventArgs) Handles Cmd_PendingBill.Click
        ...
        'Dim ChildSql As String
        'SQLSTRING = "SELECT DISTINCT KOTDETAILS,KOTDATE,MCODE,MNAME,PAYMENTTYPE,poscode,servercode FROM PENDINGKOTREGISTER WHERE CAST(Convert(varchar(11),KOTDATE,6) AS DATETIME) = '" & Format(dtp_BillDate.Value, "dd-MMM-yyyy") & "' "
        ...
        Dim ChildSql As String
        SQLSTRING = "SELECT DISTINCT ORDERDETAILS,ORDERDATE,MCODE,MNAME,PAYMENTTYPE,poscode,servercode FROM PENDINGKOTREGISTER WHERE CAST(Convert(varchar(11),ORDERDATE,6) AS DATETIME) = '" & Format(dtp_BillDate.Value, "dd-MMM-yyyy") & "' "
        ChildSql = "SELECT KOTDETAILS,CATEGORY,GROUPCODE,ITEMTYPE,POSCODE,ITEMCODE,ITEMDESC,UOM,QTY,RATE,AMOUNT,TAXAMOUNT,PAYMENTMODE,ISNULL(PACKAMOUNT,0) AS SERCHARGE,ISNULL(TipsAmt,0) AS TipsAmt,ISNULL(ADCGSAMT,0) AS ADDITIONCGS,ISNULL(PARTYAMT,0) AS PARTAMT,ISNULL(ROOMAMT,0) AS ROOMAMT FROM KOT_DET"
        gconnection.getDataSet(SQLSTRING, "KOT_HDR")
        OBJ1.LOADGRID(gdataset.Tables("KOT_HDR"), True, "FRM_TKGA_SmartKotEntry", ChildSql, "KOTDETAILS", 1)
        ' Me.Close()
