...
Imports System.Data
Imports System.IO
Public Class FRM_MCOS_ItemMaster
    ...
    Private Sub FRM_MCOS_ItemMaster_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles Me.KeyDown
        ...
        End If
    End Sub
    Private Sub FRM_MCOS_ItemMaster_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
    ...
    Private Sub txtItemCode_Validated(ByVal sender As Object, ByVal e As System.EventArgs) Handles txtItemCode.Validated
        ...
        If Trim(txtItemCode.Text) <> "" Then
            ...
            ssql = ssql & " ISNULL(BASERATE,0) AS BASERATE,ISNULL(GuestRate,0) as GuestRate,ISNULL(MRPTAG,'') AS MRPTAG,ISNULL(MRPRATE,0) AS MRPRATE,isnull(LWTFLAG,'') as LWTFLAG,isnull(fbflag,'') fbflag, ISNULL(STARTINGDATE,GETDATE()) AS STARTINGDATE,ISNULL(ENDINGDATE,GETDATE()) AS ENDINGDATE,"
            ...
            If gdataset.Tables("ItemMaster").Rows.Count > 0 Then
                ...
                If Trim(gdataset.Tables("ItemMaster").Rows(0).Item("MULTIRATE")) = "Y" Then
                    ...
                    txt_BaseGuestRate.Text = Format(Val(gdataset.Tables("ItemMaster").Rows(0).Item("GuestRate")), "0.00")
                    cbo_BaseUOM.DropDownStyle = ComboBoxStyle.DropDownList
                    With ssgrid
                        ssql = "SELECT ISNULL(UOM,'') AS UOM,ISNULL(ITEMRATE,0) AS ITEMRATE,ISNULL(GuestRate,0) as GuestRate,ISNULL(MRPRATE,0) AS MRPRATE,ISNULL(PURCAHSERATE,0) AS PURCAHSERATE,ISNULL(RPOSCODE,'') AS POS FROM RateMaster WHERE Itemcodeseqno=" & (vseqno) & " AND ISNULL(ENDINGDATE,'')='' "
                        ...
                        If gdataset.Tables("RateMaster").Rows.Count > 0 Then
                            For i = 0 To gdataset.Tables("RateMaster").Rows.Count - 1
                                ...
                                .SetText(7, i + 1, Val(gdataset.Tables("RateMaster").Rows(i).Item("GuestRate")))
                            Next
                        End If
                    End With
                Else
                    ...
                    FraGrid.Visible = False
                    FraRate.Visible = True
                    ssql = "SELECT ISNULL(UOM,'') AS UOM,ISNULL(ITEMRATE,0) AS ITEMRATE,ISNULL(GuestRate,0) as GuestRate,ISNULL(MRPRATE,0) AS MRPRATE,ISNULL(PURCAHSERATE,0) AS PURCAHSERATE FROM RateMaster WHERE Itemcodeseqno=" & (vseqno) & " and ISNULL(ENDINGDATE,'')='' "
                    vconn.getDataSet(ssql, "RateMaster")
                    If gdataset.Tables("RateMaster").Rows.Count > 0 Then
                        ...
                        txtGuestRate.Text = Format(Val(gdataset.Tables("RateMaster").Rows(0).Item("GuestRate")), "0.00")
    ...
    Private Sub CmdAdd_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles CmdAdd.Click
        ...
        If Mid(CmdAdd.Text, 1, 1) = "U" Then
            ...
            If OptNo.Checked = True Then
                ...
                ssql = ssql & " BaseUOMstd = '" & Trim(CmbUOm.Text) & "',BaseRatestd=" & Format(Val(txtRate.Text), "0.00") & ",GuestRate=" & Format(Val(txtGuestRate.Text), "0.00") & ","
            Else
                ssql = ssql & " BaseUOMstd = '" & Trim(cbo_BaseUOM.Text) & "',BaseRatestd=" & Format(Val(txt_BaseRate.Text), "0.00") & ",GuestRate=" & Format(Val(txt_BaseGuestRate.Text), "0.00") & ","
            End If
    ...
    Private Sub InsertPromotional(ByVal Insert() As String)
        ...
        If Mid(CmdAdd.Text, 1, 1) = "A" Then
            ...
            ssql = ssql & " Freeze,Multirate,BaseUOMstd,BaseRatestd,GuestRate,BaseRate,mrptag,MRPRate,BaseUOM,BaseQty,StockControl,PromItemcode,"
            ...
            If optNo.Checked = True Then
                ssql = ssql & " '" & Trim(CmbUOm.Text) & "'," & Format(Val(txtRate.Text), "0.00") & "," & Format(Val(txtGuestRate.Text), "0.00") & ","
            Else
                ssql = ssql & " '" & Trim(cbo_BaseUOM.Text) & "'," & Format(Val(txt_BaseRate.Text), "0.00") & "," & Format(Val(txt_BaseGuestRate.Text), "0.00") & ","
            End If
            ssql = ssql & " 0,"
            If CHK_MRPTAG.Checked = True Then
                ssql = ssql & " 'Y',"
        ...
        For i = 0 To LstPOScode.Items.Count - 1
            If LstPOScode.GetItemChecked(i) = True Then
                ...
                vseqno = GetSeqno(txtItemCode.Text)
                'ssql = " INSERT INTO POSMenulink (ItemCodeseqno,Itemcode,POS,POSseqno) "
                ssql = " INSERT INTO RateMaster(ItemCodeseqno,ItemCode,UOM,ItemRate,GuestRate,PRITEMRATE,purcahseRate,MRPRate,"
                ssql = ssql & " Startingdate,AddUserId,AddDateTime,rposcode)"
                ssql = ssql & " VALUES( " & (vseqno) & ","
                ssql = ssql & " '" & Trim(txtItemCode.Text) & "','" & Trim(CmbUOm.Text) & "',"
                ssql = ssql & " " & Format(Val(txtRate.Text), "0.00") & "," & Format(Val(txtGuestRate.Text), "0.00") & ",0.00,"
                'ssql = ssql & "0.00,"
    ...
    Private Sub InsertGridYes(ByVal Insert() As String)
        ...
        If Mid(CmdAdd.Text, 1, 1) = "A" Then
            ...
            vseqno = GetSeqno(txtGroupcode.Text)
            ssql = ssql & " " & Trim(vseqno) & ","
            ssql = ssql & " 'N','Y','" & Trim(cbo_BaseUOM.Text) & "'," & Format(Val(txt_BaseRate.Text), "0.00") & "," & Format(Val(txt_BaseGuestRate.Text), "0.00") & ","
            ssql = ssql & " 0,"
            If CHK_MRPTAG.Checked = True Then
                ssql = ssql & " 'Y',"
            Else
                ...
                If LstPOScode.GetItemChecked(j) = True Then
                    ...
                    For i = 1 To ssgrid.DataRowCnt
                        ...
                        With ssgrid
                            ...
                            vseqno = GetSeqno(txtItemCode.Text)
                            'ssql = " INSERT INTO POSMenulink (ItemCodeseqno,Itemcode,POS,POSseqno) "
                            ssql = " INSERT INTO RateMaster(ItemCodeseqno,ItemCode,UOM,ItemRate,GuestRate,MRPRate,purcahseRate,PritemRate,"
                            ssql = ssql & " Startingdate,AddUserId,AddDateTime,rposcode)"
                            ssql = ssql & " VALUES(" & (vseqno) & ","
                            ssql = ssql & " '" & Trim(txtItemCode.Text) & "'," '" & Trim(CmbUOm.Text) & "',"
                            .Row = i
    ...
    Private Sub txt_BaseGuestRate_KeyPress(sender As Object, e As KeyPressEventArgs) Handles txt_BaseGuestRate.KeyPress
        If Not Char.IsControl(e.KeyChar) AndAlso Not Char.IsDigit(e.KeyChar) AndAlso e.KeyChar <> "."c Then
            e.Handled = True
        End If

        'for ne decimal point
        If (e.KeyChar = "."c AndAlso (txtGuestRate).Text.IndexOf("."c) > -1) Then
            e.Handled = True
        End If
        If Asc(e.KeyChar) = 13 Then
            Dim sqlstring As String
            ...
            If CmdAdd.Text = "Update[F7]" Then
                For i = 1 To ssgrid.DataRowCnt
                    ssgrid.Row = i
                    ssgrid.Col = 7
                    If Trim(txt_BaseGuestRate.Text) <> Trim(ssgrid.Text) Then
                        ssgrid.SetText(7, i, Format(Val(txt_BaseGuestRate.Text), "0.00"))
                    End If
                Next i
                For j = 1 To ssgrid.DataRowCnt
                    ssgrid.Col = 7
                    ...
                    vconn.getDataSet(sqlstring, "UOMRELATION")
                    If gdataset.Tables("UOMRELATION").Rows.Count > 0 Then
                        dblrate = gdataset.Tables("UOMRELATION").Rows(0).Item("ratio2") * Val(txt_BaseGuestRate.Text)
                        ssgrid.SetText(7, j, Format(Val(dblrate), "0.00"))
                    End If
                Next j
                ssgrid.SetActiveCell(1, ssgrid.ActiveRow)
                ssgrid.Focus()
        ...
        End If
    End Sub
    Private Sub txt_BaseGuestRate_LostFocus(sender As Object, e As EventArgs) Handles txt_BaseGuestRate.LostFocus
        txt_BaseGuestRate.Text = Format(Val(txt_BaseGuestRate.Text), "0.00")
    End Sub

    Private Sub txt_BaseGuestRate_TextChanged(sender As Object, e As EventArgs) Handles txt_BaseGuestRate.TextChanged
        Dim sqlstring As String
        Dim dblrate As Double
        Dim i, j As Integer
        Dim vuom As String
